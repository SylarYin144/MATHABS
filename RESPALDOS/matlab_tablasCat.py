...
class TablasCat(ttk.Frame):
    def __init__(self, master, main_app_instance=None, log_callback=None):
    ...
    def create_widgets(self):
        ...
        # 3. Sección de Filtros Generales
        frm_filters = ttk.LabelFrame(self.scrollable_controls, text="Filtros Generales")
        frm_filters.pack(fill=tk.X, padx=5, pady=5)
        if FilterComponent:
            self.filter_component = FilterComponent(frm_filters, log_callback=self.log_func)
            self.filter_component.pack(fill=tk.BOTH, expand=True)
        else:
            ttk.Label(frm_filters, text="Componente de filtro no disponible.").pack()
        ...
        btn_move_to_qual = ttk.Button(frm_quant, text="Convertir a Cualitativa", command=self.move_to_qualitative)
        btn_move_to_qual.pack(padx=5, pady=5)
        ...
        btn_move_to_quant = ttk.Button(frm_qual, text="Convertir a Cuantitativa", command=self.move_to_quantitative)
        btn_move_to_quant.pack(padx=5, pady=5)
    ...
    def move_to_qualitative(self):
        selected_indices = self.lst_quant.curselection()
        if not selected_indices:
            messagebox.showinfo("Información", "Selecciona una variable de la lista de cuantitativas para mover.")
            return

        for i in selected_indices[::-1]: # Iterar hacia atrás para evitar problemas con los índices
            var_name = self.lst_quant.get(i)
            self.lst_quant.delete(i)
            self.lst_qual.insert(tk.END, var_name)
            self.log_func(f"Variable '{var_name}' movida de cuantitativa a cualitativa.", "INFO")

    def move_to_quantitative(self):
        selected_indices = self.lst_qual.curselection()
        if not selected_indices:
            messagebox.showinfo("Información", "Selecciona una variable de la lista de cualitativas para mover.")
            return

        for i in selected_indices[::-1]: # Iterar hacia atrás para evitar problemas con los índices
            var_name = self.lst_qual.get(i)
            # Verificación básica para asegurar que la variable pueda ser tratada como numérica
            if self.data is not None and pd.api.types.is_numeric_dtype(self.data[var_name]):
                self.lst_qual.delete(i)
                self.lst_quant.insert(tk.END, var_name)
                self.log_func(f"Variable '{var_name}' movida de cualitativa a cuantitativa.", "INFO")
            else:
                messagebox.showwarning("Advertencia", f"La variable '{var_name}' no parece ser numérica y no se puede mover.")
    ...
    def update_variable_lists(self):
        if self.main_app and hasattr(self.main_app, 'data_filter_tab') and self.main_app.data_filter_tab.data is not None:
            ...
            if FilterComponent and hasattr(self, 'filter_component'):
    ...
    def _apply_filters_and_categorization(self):
        ...
        if FilterComponent and hasattr(self, 'filter_component'):
            ...
            if df is None:
                self.log_func("El filtro no devolvió datos.", "WARN")
                return None, None
        else:
            df = self.data.copy()
    ...
    def generate_composite_table(self):
        ...
        if cat_var and cat_var in df_filtered.columns:
            ...
            if ordered_categories_from_filter: # Use the variable from the function call
                 ...
                 cat_values = sorted(list(self.data[cat_var].dropna().unique()))
            else:
                 # Tomar del DF filtrado y ordenar alfabéticamente
                 cat_values = sorted(list(df_filtered[cat_var].dropna().unique()))
